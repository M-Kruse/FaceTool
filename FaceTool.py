#!/usr/bin/python
"""
This is a python class for using dlib, and eventually other libraries,
for various image/face processing tasks.
"""
...
class FaceTool():
    ...
    def __init__(self, shape_model, face_rec_model, keyspace, cluster_node_ip_list):
        ...
        self.session.execute("""
            CREATE INDEX IF NOT EXISTS ON {0}.images (image_url)""".format(keyspace))
        ...
        self.session.execute("""
            CREATE TABLE IF NOT EXISTS {0}.identities (
                ...
                PRIMARY KEY (id))""".format(self.keyspace))
        self.session.execute("""
            CREATE INDEX IF NOT EXISTS ON {0}.identities (source_encodings)""".format(keyspace))

    def get_source_images(self, images_dir):
        """
        Glob images based on listed image types.
        """
    ...
    def extract_face_descriptors(self, image_path):
        """
        Loads an image, extracts and returns the face descriptors from it.
        """
    ...
    def cluster_face_descriptors(self, threshold=0.5):
        """
        Takes a list of face descriptors and clusters them with
        dlib's chinese whisper cluster method to determine total
        unique identities in an image
        """
        self.labels = dlib.chinese_whispers_clustering(self.face_descriptors, threshold)
        num_classes = len(set(self.labels))
        print("Total face descriptors: {0}".format(str(len(self.face_descriptors))))
    ...
    def compare_faces(self, face_a_vectors, face_b_vectors, distance_threshold=0.6):
        """
        Computation for comparing two face descriptors, returns True if within threshold.
        #https://github.com/davisking/dlib/issues/950#issuecomment-353690061
        """
        dist = sum((face_a_vectors[dim] - face_b_vectors[dim]) ** 2 for dim in range(len(face_a_vectors)))
    ...
    def db_add_image(self, source, query_str, query_page_num, image_url, alt_text, storage_path):
        """
        Inserts new images and metadata to the database
        """
        log_time = time.time()
        #I can't get this to work without using the old style string sub
        # while the other function works with using "{}".format() ?????
        new_uuid = util.uuid_from_time(log_time)
        query = "INSERT INTO " + self.keyspace + ".images (id, source, query, query_page, image_url, alt_text, storage_path) VALUES (%s, %s, %s, %s, %s, %s, %s) IF NOT EXISTS", [new_uuid, source, query_str, query_page_num, image_url, alt_text, storage_path]
        result = self.session.execute()
        new_row = result[0]['[applied]']
        if new_row:
            print("[INFO] Added image {} to DB".format(image_url))
    ...
    def db_add_face_descriptors(self, new_face_descriptor, source_img_uuid):
        """
        Inserts new face descriptors to the database
        """
        log_time = time.time()
        new_uuid = util.uuid_from_time(log_time)
    ...
    def db_get_all_encodings(self):
        """
        Returns a list of all the face encodings in the database
        """
        return self.session.execute("SELECT * FROM FACETOOL.faces")

    def db_search_source_img_by_uuid(self, source_uuid):
        """
        Searches db for an image UUID to get the row data
        """
    ...
    def db_search_for_face(self, target_face_descriptor):
        """
        Searches db for an existing face descriptor
        """
        results = []
        for row in self.db_get_all_encodings():
            test_face = row['face_encoding']
            if self.compare_faces(target_face_descriptor, test_face):
                results.append(row)
    ...
    def db_search_img_url(self, url):
        """
        Searches the db for existing URL
        """
    ...
    def is_single_face_image(self, image_path):
        """
        Detects if an image has a single face
        """
